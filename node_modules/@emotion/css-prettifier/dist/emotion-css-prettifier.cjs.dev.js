...
var prettyStringify = memoize__default["default"](function (indentation) {
  return function (element, index, children, callback) {
    switch (element.type) {
      ...
      case '@media':
      case '@supports':
        element.value = stylis.combine(stylis.tokenize(element.value), function (value, index, children) {
          // (
    ...
    return (children = stylis.serialize(element.children, callback)).length ? element["return"] = element.value + ' {\n' + children.trim().replace(/^/gm, indentation).replace(/^\s+$/gm, '') + '\n}\n\n' : '';
  };
});
function prettify(styles, indentation) {
  ...
  return stylis.serialize(stylis.compile(styles), prettyStringify(indentation)).trim();
}
